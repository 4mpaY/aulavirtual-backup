...
export class UnenrolledError extends Error {
  code = 'UNCISCRIBED' as const
}

export async function getPlayerCourseData(user: { id: string; rol: string }, slug: string) {
  ...
  if (!course) {
    throw new CourseNotFoundError('Curso no encontrado')
  }
  ...
  if (!isAdmin && !isCourseProfessor) {
    ...
    if (!acceso) {
      throw new UnenrolledError('No tienes acceso a este curso')
    }
