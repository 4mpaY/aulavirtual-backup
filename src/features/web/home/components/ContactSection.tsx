...
export default function ContactSection() {
  const [formData, setFormData] = useState({
    name: '', email: '', phone: '', area: 'ingenieria', service: '', message: '',
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    window.open(`https://wa.me/${selectedPhone}?text=${text}`, '_blank')
    setSent(true)
      ...
      <div className="container-custom mx-auto">
        ...
        <div className="text-center mb-14">
          ...
          <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold text-[#0D3A52] mb-4">
            ¿Tienes un <span className="text-[#1177AB]">proyecto</span> en mente?
          </h2>
        ...
        <div className="grid lg:grid-cols-5 gap-10 mb-16">
          ...
          <div className="lg:col-span-3">
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-8 shadow-sm border border-[#EBF5FB] h-full flex flex-col">
              {sent && (
                <div className="mb-5 p-4 rounded-xl bg-[#1177AB]/10 border border-[#1177AB]/20 text-[#1177AB] font-semibold text-sm">
                    ...
                    <div>
                      ...
                      {href ? (
                        <a href={href} target={href.startsWith('http') ? '_blank' : undefined} rel="noopener noreferrer"
                          className="text-sm text-[#4d6b7d] hover:text-[#1177AB] transition-colors">
                          {value}
                        </a>
                      ) : (
